...
<head>
    <title>فعالیت ها - iVideo</title> <!-- عنوان صفحه -->
    <?php
...
<body>
                ...
                <div class="col-lg-9 col-md-7">
                    ...
                    <div class="simple-info">
                        ...
                        <div class="section-title ">
                            <h2>آخرین فعالیت ها</h2> <!-- عنوان بهترین ویدیو ها  -->
						</div>
    ...
    <tbody>
    <?php 
     $S_L_M = mysqli_query($DB_config, "SELECT * from contact_us ORDER BY `Timestamp` desc limit 100 ");
     while ($S_L_M_row = mysqli_fetch_array($S_L_M)) {
        $S_L_M_Author = $S_L_M_row['Author'];
        $S_L_M_Subject = $S_L_M_row['Subject'];
        $S_L_M_Massege = $S_L_M_row['Massege'];
        $S_L_M_Timesptamp = $S_L_M_row['Timestamp'];
         ?>
      <tr>
        <td> <?=$S_L_M_Author?></td>
        <td><?=$S_L_M_Subject?></td>
        <td><?=$S_L_M_Massege?></td>
        <td><?=$S_L_M_Timesptamp?></td>
      </tr>
     <?php }?>
    </tbody>
  </table>
